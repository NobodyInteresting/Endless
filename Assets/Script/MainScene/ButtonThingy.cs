...
public class ButtonThingy : MonoBehaviour {
    ...
    private void Start()
    {
        if (PlayerPrefs.GetInt("M") == 2)//if music is off, keep it that way
        {
    ...
    private void Update()
    {
        ...
        {
            if ((Input.GetTouch(0).position.y >= (GameObject.Find("Rate").GetComponent<Transform>().position.y + 20f) || Input.GetTouch(0).position.y <= (GameObject.Find("Rate").GetComponent<Transform>().position.y - 20f)))
            {
                Begin();
            }
        }//try to factor in x coordinates
